...
import { NoiPropertySelect } from "@/components/owner/noi-property-select";
...
function noiHref(propertyId: string, range: string) {
  const qs = new URLSearchParams();
  qs.set("property", propertyId);
  if (range !== "month") qs.set("range", range);
  return `/owner/noi?${qs.toString()}`;
}

export default async function OwnerNoiPage({
  searchParams,
}: {
  searchParams: Promise<{ range?: string; property?: string }>;
}) {
  const params = await searchParams;
  const range = parseNoiRange(params.range);
  const selectedPropertyId = params.property?.trim() || null;
      ...
      <div className="space-y-4">
        ...
        {propertyError ? (
          ...
          <p className="text-sm text-slate-600">
            No properties are linked to this owner yet.
          </p>
        )}
      </div>
    );
  ...
  const activeProperty =
    (selectedPropertyId
      ? propList.find((p) => p.id === selectedPropertyId)
      : null) ?? propList[0];

  if (!selectedPropertyId || selectedPropertyId !== activeProperty.id) {
    redirect(noiHref(activeProperty.id, range));
  }
  ...
  const [{ data: invoices }, { data: costs }, { data: labor }, { data: statements }] =
    await Promise.all([
      supabase
        .from("invoices")
        .select(
          "id, property_id, total, status, party_type, period_end, due_date, issue_date, invoice_lines(line_type, amount)"
        )
        .eq("property_id", activeProperty.id)
        .eq("party_type", "tenant"),
      supabase
        .from("cost_entries")
        .select("id, property_id, category, description, amount, incurred_date")
        .eq("owner_id", ownerId!)
        .eq("property_id", activeProperty.id)
        .order("incurred_date", { ascending: false }),
      supabase
        .from("labor_time_entries")
        .select("id, labor_cost, notes, work_date, hours, property_id")
        .eq("property_id", activeProperty.id)
        .order("work_date", { ascending: false }),
      supabase
        .from("owner_statements")
        .select("management_fee, period_start, period_end, status, property_id")
        .eq("owner_id", ownerId!)
        .eq("property_id", activeProperty.id),
    ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            {activeProperty.address_line1}, {activeProperty.city},{" "}
            {activeProperty.state}
          </p>
        </div>
        <div className="flex flex-wrap items-end gap-3">
          <NoiPropertySelect
            selectedId={activeProperty.id}
            range={range}
            properties={propList.map((p) => ({ id: p.id, name: p.name }))}
          />
          <NoiRangePills
            selected={range}
            basePath="/owner/noi"
            preserveParams={{ property: activeProperty.id }}
          />
        </div>
      </div>
      ...
      <p className="text-sm text-slate-500">
        ...
        {start} → {end}
        {" · "}
        <span className="font-medium text-slate-700">{activeProperty.name}</span>
      </p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-amber-200/80 bg-amber-50/70 px-4 py-3">
          <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-900/70">
            OpEx
            <MetricInfoTip label="OpEx" detail={METRIC_EXPLAINERS.opex} />
          </p>
        ...
        <Card title="Income mix">
          <NoiMixBars
            items={[
              ...
              {
                label: "Other income (CAM, fees, etc.)",
                amount: otherIncome,
              },
            ]}
