...
import { deleteOwnerManagerMessages } from "@/app/owner/actions";

export type OwnerContactMessage = {
  id: string;
  sender_role: string;
  sender_name: string;
  body: string;
...
export function OwnerContactConversation({
  messages,
}: {
  messages: OwnerContactMessage[];
}) {
  const [selecting, setSelecting] = useState(false);
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [pending, startTransition] = useTransition();

  const hasOwnerMessages = messages.some((m) => m.sender_role === "owner");
  ...
  function removeSelected() {
    ...
    startTransition(async () => {
      await deleteOwnerManagerMessages(formData);
      exitSelect();
    });
  }
      ...
      {messages.length === 0 ? (
        ...
        <div>
          <ul className="space-y-5">
            {messages.map((m) => {
              const fromOwner = m.sender_role === "owner";
                    ...
                    >
                      <p
                        className={`text-xs font-medium ${
                          fromOwner ? "text-slate-300" : "text-slate-500"
                        }`}
                      >
