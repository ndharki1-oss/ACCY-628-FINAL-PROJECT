...
export async function GET(
  ...
  const { data: invoice, error } = await supabase
    .from("invoices")
    .select(
      "id, invoice_number, party_type, status, issue_date, due_date, total, amount_paid, dispute_reason, invoice_lines(line_type, description, amount), properties(name, address_line1, city, state, postal_code), tenants(company_name, contact_name, email)"
    )
    .eq("id", invoiceId)
