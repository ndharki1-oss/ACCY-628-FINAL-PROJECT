...
export default async function AdminPropertyOwnersPage() {
  ...
  const { data: propertyOwners } = await supabase
    .from("properties")
    .select(
      "id, name, owner_id, owners(id, contact_name, company_name, email, phone)"
    )
    .order("name");
      ...
      <Card title="Property Owners">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {(propertyOwners ?? []).map((property) => {
                const owner = firstRelation(property.owners);
                const ownerId = owner?.id ?? property.owner_id;
                const phone = ownerDisplayPhone(ownerId, owner?.phone);
                const preferred = ownerPreferredContact(ownerId);
