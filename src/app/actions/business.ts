...
export async function adminAssignWorkOrder(formData: FormData) {
  ...
  // Escalated / owner-approval path must go to Victor (contractor), never staff.
  if (routing.requiresOwnerApproval || wo.requires_owner_approval) {
    vendorId = DEMO_CONTRACTOR_VENDOR_ID;
  }
  ...
  await supabase.rpc("write_audit", {
    ...
    p_detail: {
      ...
      overThresholdForcedContractor:
        routing.requiresOwnerApproval || Boolean(wo.requires_owner_approval),
    },
  });
