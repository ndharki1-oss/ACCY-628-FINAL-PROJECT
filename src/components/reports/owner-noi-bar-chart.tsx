...
export function OwnerNoiBarChart({
  activity,
  owners,
  ownerFilter,
  onOwnerFilterChange,
  ...
  const aggregated = useMemo(
    () =>
      aggregateOwnerPnLChart(activity, {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-600">
            {count === 0
              ? `All owners by ${metricName}`
              : `All ${count} owner${count === 1 ? "" : "s"} · sorted by ${metricName}`}
          </p>
        </div>
        ...
        {count > 0 ? (
          ...
          <div className="flex h-full items-center justify-center rounded border border-dashed border-slate-200 bg-[#fdfbf7] text-sm text-slate-500">
            {preset === "custom" && (!customStart || !customEnd)
              ...
              : "No owners match the current chart filters."}
          </div>
        )}
      </div>
    </div>
