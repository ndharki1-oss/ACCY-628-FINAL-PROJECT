...
import {
  dismissEmployeeNotification,
  hrefForEmployeeNotification,
  loadEmployeeNotifications,
  seedExampleEmployeeNotifications,
  syncDerivedEmployeeNotifications,
  EMPLOYEE_NOTIFICATIONS_EVENT,
  type AssignmentHint,
  type EmployeeNotification,
} from "@/lib/employee-notifications-store";
...
export function EmployeeNotifications({
  ...
  useEffect(() => {
    seedExampleEmployeeNotifications();
    syncDerivedEmployeeNotifications(assignmentHints);
    setNotifications(loadEmployeeNotifications());
    // Serialize so stable server payloads don't thrash on new array identity.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional
  }, [JSON.stringify(assignmentHints)]);

  useEffect(() => {
    const sync = () => setNotifications(loadEmployeeNotifications());
    window.addEventListener(EMPLOYEE_NOTIFICATIONS_EVENT, sync);
    window.addEventListener("storage", sync);
    return () => {
      window.removeEventListener(EMPLOYEE_NOTIFICATIONS_EVENT, sync);
  ...
  function handleNotificationClick(id: string) {
    dismissEmployeeNotification(id);
    setNotifications(loadEmployeeNotifications());
    setOpen(false);
  }
      ...
      {open ? (
        ...
        >
          <div className="border-b border-slate-100 px-4 py-3">
            ...
            <p className="text-xs text-slate-500">Updates from Assignments</p>
          </div>
          ...
          {notifications.length === 0 ? (
            ...
            <ul className="max-h-80 overflow-y-auto">
              {notifications.map((n) => {
                const href = hrefForEmployeeNotification(n);
                const body = (
                  <>
