...
export function OwnerNotifications({
  ...
  useEffect(() => {
    ...
    window.addEventListener(OWNER_NOTIFICATIONS_EVENT, sync);
    window.addEventListener("storage", sync);
    return () => {
      window.removeEventListener(OWNER_NOTIFICATIONS_EVENT, sync);
        ...
        >
          ...
          {notifications.length === 0 ? (
            ...
            <ul className="max-h-80 overflow-y-auto">
              {notifications.map((n) => {
                const href = hrefForOwnerNotification(n);
                const body = (
                  <>
